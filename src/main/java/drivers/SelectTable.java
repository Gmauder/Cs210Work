...
public class SelectTable implements Driver {
	private static final Pattern pattern = Pattern.compile(
			//CREATE\s+TABLE\s+([a-z0-9_]+)\s+AS\s+(SELECT\s*(?:\*|(?:(?:[a-z0-9_]+(?:(?:(?:\s*AS\s*[a-z0-9_]+)*\s*)\s*)?))(?:(?:\s*,\s*[a-z0-9_]+(?:(?:(?:\s*AS\s*[a-z0-9_]+)*\s*)\s*)?))*|[a-z0-9_]+\s*AS\s*[a-z0-9_]+)\s*FROM\s*([a-z0-9_]+)(?:\s*(\s*WHERE\s*([a-z0-9_]+)\s*(<|<>|>|=|>=|<=)\s*([a-z0-9"]+))?))
		"CREATE\\s+TABLE\\s+([a-z0-9_]+)\\s+AS\\s+(SELECT\\s*(?:\\*|(?:(?:[a-z0-9_]+(?:(?:(?:\\s*AS\\s*[a-z0-9_]+)*\\s*)\\s*)?))(?:(?:\\s*,\\s*[a-z0-9_]+(?:(?:(?:\\s*AS\\s*[a-z0-9_]+)*\\s*)\\s*)?))*|[a-z0-9_]+\\s*AS\\s*[a-z0-9_]+)\\s*FROM\\s*([a-z0-9_]+)(?:\\s*(\\s*WHERE\\s*([a-z0-9_]+)\\s*(<|<>|>|=|>=|<=)\\s*([a-z0-9\"]+))?))",
	...
	public boolean parse(String query) throws QueryError {
		...
		oldName = matcher.group(3);
		selectQuery = matcher.group(2);
	...
	public Object execute(Database db) throws QueryError {
		...
		Table selectTable = (Table) db.interpret(selectQuery);
		selectTable.setTableName(tableName);
		
		db.create(selectTable);
